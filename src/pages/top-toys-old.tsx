...
export default function TopBrinquedos() {
    ...
    useLayoutEffect(() => {
        ...
        if (textElement) {
            // Divide o texto em palavras com SplitType
            new SplitType(textElement, { types: "words", tagName: "span" });
            const words = textElement.querySelectorAll(".word");
            gsap.set(words, { y: -60, opacity: 0 });

            // Cria o ScrollTrigger para controlar as animações
            ScrollTrigger.create({
                trigger: textElement,
                start: "top 60%",
                end: "bottom 20%",
                markers: true,
                // Animação de entrada com elastic.out
                onEnter: () => {
                    gsap.to(words, {
                        y: 0,
                        opacity: 1,
                ...
                // Animação de saída com ease normal e animação dos últimos para os primeiros
                onLeaveBack: () => {
                    gsap.to(words, {
                        y: -60,
                        opacity: 0,
    ...
    return (
        ...
        >
            <div className="text-center mb-8">
                <h1 className="titulo-brinquedos text-xl lg:text-4xl font-bold mb-2 text-primary z-[-1]">Top Brinquedos Mais Alugados</h1>
